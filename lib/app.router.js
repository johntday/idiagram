...
Router.map(function() {
    ...
    this.route('landingRoute', {
        path: '/',
        waitOn: function(){
            return [ Meteor.subscribe('diagram_top5'), Meteor.subscribe('tag_userId') ];
        },
        action: function () {
            renderHomePage(this);
        },
    ...
    this.route('seqRoute', {
        ...
        template: 'seqDgmPage',
        waitOn: function () {
            return [ Meteor.subscribe('diagram_id', this.params._id), Meteor.subscribe('tag_docId', this.params._id) ];
        },
        data  : function () {
    ...
    this.route('viewRoute', {
        ...
        template: 'seqDgmView',
        waitOn: function () {
            return [ Meteor.subscribe('diagram_id', this.params._id), Meteor.subscribe('tag_id', this.params._id) ];
        },
        data  : function () {
            return Diagrams.findOne(this.params._id);
        },
    ...
    this.route('seqListRoute', {
        ...
        template: 'seqDgmList',
        waitOn: function(){
            return Meteor.subscribe('tag_userId');
        },
        onAfterAction: function(){
    ...
    this.route('seqListParmsRoute', {
        ...
        template: 'seqDgmList',
        onBeforeAction: function(){
            AppProperties.diagrams.filters.isStar = this.params.isStar == 1;
            //AppProperties.diagrams.filters.text = text;
            AppProperties.diagrams.filters.private = this.params.private;
            AppProperties.diagrams.filters.hideOtherPublic = this.params.hideOtherPublic == 1;
            AppProperties.diagrams.sort.sortBy = this.params.sortBy;
            AppProperties.diagrams.sort.sortDir = this.params.sortDir;
    ...
    this.route('tagListRoute', {
        path: '/tags',
        template: 'tagList',
        onAfterAction: function(){
            document.title = "Tag List";
        }
    });
    this.route('tagRoute', {
        path: '/tag/:_id',
        template: 'tagPage',
        waitOn: function () {
            return Meteor.subscribe('tag_id', this.params._id);
        },
        data  : function () {
            return Historys.findOne(this.params._id);
        },
