...
Router.map(function () {
    ...
    this.route('diagramRoute', {
        path: '/diagram/:_id',
        waitOn: function () {
            return Meteor.subscribe('diagram_id', this.params._id);
        },
        action: function(){
            var data = Diagrams.findOne(this.params._id);
            ...
            if (data) {
                if (data.type == 'ctx') {
                    ...
                    this.render("eastPanel", {to: 'east'});
                    this.render("westPanel", {to: 'west'});
                    this.render('seqDgmPage', {
                        data: function () {
                            return data;
                        }
                    });
    ...
    // TESTING
    this.route('colaRoute', {
        path: '/cola',
        template: 'cola',
        onAfterAction: function(){
            document.title = "Cola Test";
        }
    });
    this.route('d3TestRoute', {
        path: '/d3Test',
        template: 'd3Test',
        onAfterAction: function(){
            document.title = "d3 Test";
        }
    });
    this.route('graphEditorRoute', {
        path: '/graphEditor',
