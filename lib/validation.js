...
validateComment = function(comment) {
	return !comment.subject || validateFieldLength(comment.title, 'Title', 3, 128);
};

validateTag = function(tag){
    if (!tag){
        throwError('Enter something ;)');
        return false;
    }
    var tTag = tag.trim();
    if (!tTag){
        throwError('Enter something other than spaces ;)');
        return false;
    }
    if (tTag.length > 20){
        throwError('Tags must be less than 20 chars');
        return false;
    }
    return tTag;
};